...
(function() {
  var CMYK, HSL, HSV, RGB, hasKeys, isObject, isString, randomBetween, toPrecision;
  ...
  window.Color = (function() {
    ...
    Color.HSV_REGEX = /hsv\(\s*0*(?:360|3[0-5]\d|[12]?\d?\d)\s*(?:,\s*0*(?:100(?:\.0+)?|\d?\d(?:\.\d+)?)%?\s*){2}\)/;

    Color.CMYK_REGEX = /cmyk\((?:\s*(0(\.\d+)?|1(\.0+)?)\s*(?:,?)){4}\)/;
    ...
    Color.hsvToRgb = function(hsv, formatted) {
      ...
      if (isString(hsv)) {
        ...
        ref = hsl.match(/hsv\((.+?)\)/)[1].split(',').map(function(value) {
          var numeric;
          value.trim();
          numeric = parseFloat(value);
          return numeric = value.indexOf('%') >= 0 ? numeric / 100 : numeric;
        }), h = ref[0], s = ref[1], b = ref[2];
